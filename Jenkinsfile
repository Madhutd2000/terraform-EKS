pipeline {
  ...
  parameters {
    ...
    string(description: 'repo branch_name', name: 'branch_name', defaultValue: 'prom_graphana_new')
   ...
   stage('copy credentails') {
      steps {
        ...
        dir('terraform-scripts') {
              ...
              sh 'echo "iam_role_name_for_eks_cluster = \\"${TF_VAR_eks_cluster_name}-role\\"" >> ./terraform.tfvars'
              ...
              sh 'echo "cluster_autoscaler_role_name = \\"${TF_VAR_eks_cluster_name}_oidc_role\\"" >> ./terraform.tfvars'
