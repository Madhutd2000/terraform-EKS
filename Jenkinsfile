pipeline {
  ...
  parameters {
    ...
    string(description: 'repo branch_name', name: 'branch_name', defaultValue: 'Updated-code')
    ...
    string(name: 'TF_VAR_eks_cluster_name' ,defaultValue: 'eks-cluster-poc',description: 'Name of the cluster ')
    string(name: 'EFS_NAME' ,defaultValue: 'EFS',description: 'Name of the EFS to be created')
    string(name: 'TF_VAR_host_name',defaultValue: 'test.local',description: 'Name of the hostname for host feild in ingress rule')
    string(name: 'namespace_name' ,defaultValue: 'testing-ns',description: 'Name of the Namespace to be created inside the cluster')   
   ...
   stage('copy credentails') {
      steps {
        ...
        dir('terraform-scripts') {
             ...
             sh 'echo "efs_name = \\"${EFS_NAME}\\"" >> ./terraform.tfvars'
              ...
              sh 'echo "cluster_autoscaler_role_name = \\"${TF_VAR_eks_cluster_name}_cluster_autoscaler_role\\"" >> ./terraform.tfvars'
          
              sh 'echo "efs-iam-role = \\"${TF_VAR_eks_cluster_name}_EFS_CSI_DriverRole\\"" >> ./terraform.tfvars'
          
              sh 'echo "host_name = \\"${TF_VAR_host_name}\\"" >> ./terraform.tfvars'
//             sh 'echo "namespace_name = \\"${Tnamespace_name}\\"" >> ./terraform.tfvars'      
          }
         ...
         dir('helm-config') {
              ...
              sh 'echo "namespace_name: ${namespace_name}" >> ./vars.yaml'
        ...
        ansiColor('xterm') {
          ...
          dir('terraform-scripts') {
             ...
             // sh label: '', script: 'terraform init -reconfigure -backend-config="access_key=${TF_VAR_access_key}" -backend-config="secret_key=${TF_VAR_secret_key}" -backend-config="token=${TF_VAR_aws_session_token}" -backend-config "bucket=${Bucket_name}" -backend-config "dynamodb_table=${Dynamodb_table_name}" -backend-config "key=$TF_VAR_environment/$TF_VAR_eks_cluster_name/terraform.tfstate" -backend-config "region=${TF_VAR_region}"'
               sh label: '', script: 'terraform init -reconfigure -backend-config="access_key=${TF_VAR_access_key}" -backend-config="secret_key=${TF_VAR_secret_key}" -backend-config="token=${TF_VAR_aws_session_token}" -backend-config "bucket=${Bucket_name}" -backend-config "dynamodb_table=${Dynamodb_table_name}" -backend-config "key=$TF_VAR_eks_cluster_name/$TF_VAR_environment/terraform.tfstate" -backend-config "region=${TF_VAR_region}"'
          }          
       }
      }   
    }
...
//           sh "aws eks update-kubeconfig --name ${TF_VAR_eks_cluster_name} --region ${TF_VAR_region}"
          sh "ansible-playbook   k8s-apply.yaml"
        } 
     }
   }
     ...
     stage('helm delete') {
      ...
      steps {
        ansiColor('xterm') {
          ...
          dir('helm-config') {
            sh "pwd"
            sh "ansible-playbook   k8s-delete.yaml"
          }
      }
      } 
       post {
