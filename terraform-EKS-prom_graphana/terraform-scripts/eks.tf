...
resource "aws_iam_role_policy_attachment" "AutoscalerPolicyAttachment" {
  policy_arn = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:policy/AmazonEKSClusterAutoscalerPolicy"
  role       = aws_iam_role.eks_nodes.name
  depends_on = [
    aws_iam_policy.cluster_autoscaler_policy#autoscaler policy
  ]
}
